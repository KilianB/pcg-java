...
class PcgRSFastTest {
	...
	class State {
		...
		void equalGenerationSeed() {
			Pcg rng0 = new PcgRSFast(0, 0);
			Pcg rng1 = new PcgRSFast(0, 0);
		...
		void unequelGenerationSeed() {
			...
			Pcg rng0 = new PcgRSFast();
			Pcg rng1 = new PcgRSFast();
	...
	class Distance {
		...
		void reSeed() {
			...
			// Seed both instances
			rng = new PcgRSFast(seed, streamNumber);
			rng0 = new PcgRSFast(seed, streamNumber);
		}
		...
		void incompatibleGeneratosIncrement() {
			//We can't even create a rng with even inc ...
			assertThrows(IllegalArgumentException.class,()->{
